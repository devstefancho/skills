...
- Required frontmatter: `name` and `description`. The `description` is the trigger contract — write it so the agent auto-loads the skill for the right intents (include trigger phrases, EN/KO).
